...
SELECT
  ...
  sum(toUInt64OrZero(toString(`firecracker.metrics.block_rootfs.read_bytes`))) AS rootfs_read_bytes,
  sum(toUInt64OrZero(toString(`firecracker.metrics.block_rootfs.write_bytes`))) AS rootfs_write_bytes
FROM sandbox_events
